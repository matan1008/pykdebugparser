...
def handle_event(parser, events):
    ...
    if SamplerAction.SAMPLER_TH_INFO in e.sample_what:
        e.th_info = handle_thd_data(parser, [ev for ev in events if parser.trace_codes[ev.eventid] == 'PERF_THD_Data'])
    if SamplerAction.SAMPLER_USTACK in e.sample_what:
        header = handle_stk_uhdr(parser, [ev for ev in events if parser.trace_codes[ev.eventid] == 'PERF_STK_UHdr'])
